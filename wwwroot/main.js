...
function setupSearchAndFilter() {
  ...
  function filterPostings() {
    ...
    filteredPostings = allPostings.filter((posting) => {
      ...
      const matchesStatus =
        !statusFilterValue || posting.status === statusFilterValue;
...
function renderFilteredPostings() {
  ...
  if (filteredPostings.length === 0) {
    container.innerHTML =
      '<div class="no-results">No postings match your search criteria.</div>';
    return;
  }
    ...
    el.innerHTML = `
      <div class="posting-header">
        <div>
          ...
          ${
            p.company
              ? `<span class="posting-company">@ ${p.company}</span>`
              : ""
          }
        </div>
        <div class="posting-status">
          <span class="status-badge status-${
            p.status || "interested"
          }">${getStatusLabel(p.status || "interested")}</span>
        </div>
      </div>
      <div class="posting-details">
        ${
          p.description
            ? `<div class="posting-description">${p.description}</div>`
            : ""
        }
        ${
          p.dueDate
            ? `<div class="posting-due-date">Due: ${p.dueDate}</div>`
            : ""
        }
      </div>
      <div class="posting-actions">
        <button class="btn btn-sm" onclick="editPosting(${p.id})">Edit</button>
        <button class="btn btn-sm danger" onclick="deletePosting(${
          p.id
        })">Delete</button>
        <button class="btn btn-sm primary" onclick="tailorResume(${JSON.stringify(
          p
        ).replace(/"/g, "&quot;")})">Tailor Resume</button>
      </div>
    `;
...
function updateAnalytics() {
  ...
  const appliedCount = allPostings.filter(
    (p) =>
      p.status === "applied" ||
      p.status === "phone_screen" ||
      p.status === "interview" ||
      p.status === "offer" ||
      p.status === "rejected"
  ).length;
  const interviewCount = allPostings.filter(
    (p) => p.status === "interview" || p.status === "offer"
  ).length;
...
async function tailorResume(post) {
  console.log("TAILOR FUNCTION CALLED - FIXED VERSION"); // Debug log
  ...
  try {
    ...
    resumeContent =
      contentResponse.content || "Unable to extract resume content";
  } catch (error) {
...
Please adjust the resume info accordingly.`;
  ...
  console.log("Prompt for local AI:", prompt); // Debug log

  // Store prompt in localStorage and redirect to ai.html
...
function exportPostings() {
  const dataToExport =
    filteredPostings.length > 0 ? filteredPostings : allPostings;
  ...
  link.setAttribute(
    "download",
    `job-postings-${new Date().toISOString().split("T")[0]}.csv`
  );
